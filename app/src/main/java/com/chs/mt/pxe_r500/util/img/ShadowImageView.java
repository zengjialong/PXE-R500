 ...
 * 用法
 *
int with = (int) (getWidthPixels() * 0.75);
 LinearLayout.LayoutParams param = new LinearLayout.LayoutParams(with, with);
 param.gravity = Gravity.CENTER;
 album_cover_view.setLayoutParams(param);
 album_cover_view.startRotateAnimation();播放
 album_cover_view.pauseRotateAnimation();暂停

 DraweeController controller = Fresco.newDraweeControllerBuilder()
 .setUri(uri)
 .setOldController(simpleDraweeView.getController())
 .setControllerListener(new BaseControllerListener())
 .setTapToRetryEnabled(isRetry)
