...
import com.chs.mt.pxe_r500.datastruct.DataStruct_Inputs;
import com.chs.mt.pxe_r500.datastruct.DataStruct_Output;
...
@SuppressLint("ClickableViewAccessibility") public class EQ_INS extends View {
...
//	private String  TV_DB9="-12";//"-20"	
	...
	public  DataStruct_Inputs EQ_Filter = new DataStruct_Inputs();  
	public  DataStruct_Inputs ReadEQ = new DataStruct_Inputs();  
...
//	private  static final int  HL_OFF     = 8;                   //   高低通类型等于7时 高低通 OFF
	...
	//int EQPoint[] = new int[DataStruct.INS_CH_EQ_MAX];
    ...
    public EQ_INS(Context context, AttributeSet attrs) {  
        super(context, attrs);  
        mContext = context;
        init(attrs, 0);  
    }  
  
    public EQ_INS(Context context, AttributeSet attrs, int defStyle) {  
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //整个框的四个角
  		...
  		//计算出各占坐标
  		stepH=FrameHeight / (DataStruct.CurMacMode.INS.EQ.EQ_Gain_MAX/10);
  		stepW=Wind_Width / (float) (DataStruct.CurMacMode.INS.EQ.Max_EQ);
  		stepY=FrameHeight / (DataStruct.CurMacMode.INS.EQ.EQ_Gain_MAX/10);
...
//		}
	...
	void ComposePoint(){
		...
		for(int i=0;i<241;i++){
			m_nPoint[i]=0;
			for(int j=0;j<DataStruct.CurMacMode.INS.EQ.Max_EQ;j++){
		...
		//计算出各占坐标
		double step=(double)FrameHeight / (double)DataStruct.CurMacMode.INS.EQ.EQ_Gain_MAX;//每个DP对应多少个高度的点数
	...
	public void  SetEQ_IN_Data(DataStruct_Inputs SyncEQ) {
		for(int j=0;j<DataStruct.CurMacMode.INS.EQ.Max_EQ;j++){ 
			...
			//System.out.println(TAG+"EQ EQ_Filter.EQ["+j+"].level:"+EQ_Filter.EQ[j].freq);
		}
...
//		}
		...
		if((EQ_Filter.h_freq==20)||(EQ_Filter.h_level==7)){
			for(int	j=0;j< 241;j++){
				m_nPointHP[j]	=	0;
			}			
		}
...
//			}
		...
		if((EQ_Filter.l_freq==20000)||(EQ_Filter.l_level==7)){
			for(int	j=0;j< 241;j++){
				m_nPointLP[j]	=	0;
			}			
		}
...
//		PEQ_bw = 0.200 + (EQ_Filter.EQ[eqIndex].bw)/100.0;
		...
		PEQ_dB = (EQ_Filter.EQ[eqIndex].level-DataStruct.CurMacMode.INS.EQ.EQ_LEVEL_ZERO)/10.0;
	...
	private void dealPoint(float x, float y){
		for(int i=0;i<DataStruct.CurMacMode.INS.EQ.Max_EQ;i++){
			if((i*stepW < x )&&(i+1)*stepW>=x){
				EQ_Filter.EQ[i].level = (int)(10*(FrameHeight - y)/stepH)+DataStruct.CurMacMode.INS.EQ.EQ_LEVEL_MIN;
...
//                	for(int i=0;i<DataStruct.INS_CH_EQ_MAX_USE;i++){//清零
//                		drawPointX[i] = 0;
//                		drawPointY[i] = 0;
//                	}
//                }
...
//    }
    ...
    public interface OnEQDrawChangeListener {  
        ...
        public abstract void onProgressChanged(EQ_INS eq,  
