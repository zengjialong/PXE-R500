...
public class KnobView_APB extends ImageView {
    private static final float MAX_SWEEP_ANGLE = 120;
    private static final float START_ANGLE = -60;//
    ...
    public KnobView_APB(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        mAttrs = attrs;
        init(context);
    ...
    private void init(Context context) {
        ...
        initView();
        thumb = getResources().getDrawable(R.drawable.chs_thumb_normal);
...
//        setMeasuredDimension(min, min);
        ...
        InsideRadius = (float) (drawableRadius*0.65);
        OutsideRadius= (float) (drawableRadius*0.71);
...
//    	    System.out.println("FUCK scaleHeight:"+scaleHeight);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //drawFullCir(canvas);
        canvas.drawArc(arcRect, START_ANGLE, 360, false, progressPaint2);
        drawProgress(canvas);
        drawThumb(canvas);
    ...
    private void drawProgress(Canvas canvas){
        canvas.drawArc(arcRect, START_ANGLE-30, 30, false, progressPaint);
        //canvas.drawArc(arcRect, START_ANGLE+MAX_SWEEP_ANGLE, MAX_SWEEP_ANGLE-sweepAngle, false, progressPaint);
    ...
    private void drawLinePart(Canvas canvas) {
        ...
        for(int i=0;i<part;i++){
            in=calculatePointerPosition(i*dv, InsideRadius);
            out=calculatePointerPosition(i*dv, OutsideRadius);

            canvas.drawLine(in[0],in[1],out[0],out[1],progressPaint);
        }
    }
    ...
    private void rotateWheel(float degrees) {
        ...
        if(mProListener != null){
            mProListener.onProgressChanged(KnobView_APB.this,true, getProgress());
        }
    }
    ...
    private void setAngle(float angle, boolean callback) {
        ...
        if (wheelWidth == 0 || wheelHeight == 0) {
            ...
            if(mProListener != null){
                mProListener.onProgressChanged(KnobView_APB.this,false, getProgress());
            }
        }
    }
...
//    private float GetDegree(double x,double y){
//        float Degree=0;
//        double radian = Math.atan2(y - centerY, x - centerX);
//        if (radian < 0){
        ...
        public boolean onTouch(View v, MotionEvent event) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_DOWN:
                    ...
                    if(mProListener != null){
                        mProListener.onStartTrackingTouch(KnobView_APB.this, getProgress());
                    }
                    if (!touchable) {
                        return false;
                    }
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if(mProListener != null){
                        mProListener.onStopTrackingTouch(KnobView_APB.this, getProgress());
                    }
                    break;
            }
...
//        chs_progress = Math.max(1, chs_progress);
    ...
    public interface OnProgressChangeListener{
        ...
        void onProgressChanged(KnobView_APB view, boolean fromUser, int progress);

        void onStartTrackingTouch(KnobView_APB view, int progress);
        void onStopTrackingTouch(KnobView_APB view, int progress);
    }
}
