...
public class KnobViewPD extends ImageView {
    ...
    private static final float PROG_RATIO = 0.85f;
	...
	private float TL_inside_point_radius = 0;
	private float TL_outside_point_radius = 0;
    ...
    private float AngleEnd = AngleStart+AngleMax;
    private float mDegree  = 0;

    //图片小点功能
    private Drawable[] mIndexPress  = new Drawable[60];
    private Drawable[] mIndexNormal  = new Drawable[60];
    private int mIndexNum = 0;
    private float mIndexSize = 0;


    private Context mContext = null;
    private AttributeSet mAttrs = null;
    public KnobViewPD(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        mAttrs = attrs;
        init(context);
    ...
    private void init(Context context) {
        ...
        progressBgPaint.setColor(outside_line_color_normal);
        progressBgPaint.setAntiAlias(true);
        progressBgPaint.setStrokeCap(Paint.Cap.ROUND);


        int colorProgress = outside_line_color_press;//getResources().getColor(R.color.color_main);
        ...
        PaintInsidePoint.setColor(colorProgress);
        ...
        mDegree = AngleMax/mMax;
        invalidate();
    }
    private void initView(){
        TypedArray localTypedArray = mContext.obtainStyledAttributes(mAttrs, R.styleable.KNOB);
     	
     	inside_point_radius = localTypedArray.getDimension(R.styleable.KNOB_inside_point_radius, 6);
        ...
        mIndexNum = localTypedArray.getInt(R.styleable.KNOB_index_num, 19);
        mIndexSize = localTypedArray.getDimension(R.styleable.KNOB_index_size, 15);
        ...
        for(int i=0;i<60;i++){
            mIndexNormal[i] = getResources().getDrawable(R.drawable.chs_master_val_normal);
            mIndexPress[i] = getResources().getDrawable(R.drawable.chs_master_val_press);
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {  
        ...
        radius = (float) (drawableRadius*0.85);
        thumbRadius = radius;
        pic_radius=min / 2*PROG_RATIO;
        
        InsideRadius = (float) (drawableRadius*0.5);
        OutsideRadius= (float) (drawableRadius*0.8);
        TL_inside_point_radius = (float) (drawableRadius*0.8);
        TL_outside_point_radius = (float) (drawableRadius*0.9);
        if(!bool_matrix){
        	bool_matrix=true;
...
//        canvas.restore();
    ...
    private void drawIndex(Canvas canvas) {
        ...
        float mratio=MAX_SWEEP_ANGLE/mIndexNum;
        int index = (int) (sweepAngle/mratio);
        for(int i=0;i<mIndexNum;i++){
            float[] pointerPosition=calculatePointerPosition(305-i*mratio, OutsideRadius);
            pointerPosition[0]=pointerPosition[0]-mIndexSize/2;
            pointerPosition[1]=pointerPosition[1]-mIndexSize/2;

            if(i<=index){
                this.mIndexPress[i].setBounds((int) pointerPosition[0], (int) pointerPosition[1],
                        (int) (pointerPosition[0] + mIndexSize), (int) (pointerPosition[1] + mIndexSize));
                this.mIndexPress[i].draw(canvas);
            }else{
                this.mIndexNormal[i].setBounds((int) pointerPosition[0], (int) pointerPosition[1],
                        (int) (pointerPosition[0] + mIndexSize), (int) (pointerPosition[1] + mIndexSize));
                this.mIndexNormal[i].draw(canvas);
            }
        }
//        if(index == mIndexNum){
//            for(int i=0;i<mIndexNum;i++) {
...
//        }
    ...
    private void drawTLinePbg(Canvas canvas){
    	float angle=AngleStart+180;
    	for(int i=0;i<mMax;i++){
    		float[]Out_pointerPosition=calculatePointerPosition(angle, TL_outside_point_radius);
    		float[]In_pointerPosition=calculatePointerPosition(angle, TL_inside_point_radius);
        	angle-=mDegree;
    ...
    private void drawTLineProgress(Canvas canvas){
    	float angle=AngleStart+180;
    	for(int i=0;i<=mprogress;i++){
        	...
        	canvas.drawLine(In_pointerPosition[0], In_pointerPosition[1],
        		Out_pointerPosition[0], Out_pointerPosition[1], progressPaint );       	
    	}	
    }
    
    private void drawInsidePoint(Canvas canvas) {
    	float[] pointerPosition=calculatePointerPosition(AngleStart+180-sweepAngle, InsideRadius);   	
    ...
    private void rotateWheel(float degrees) {
        ...
        if(mProListener != null){
            mProListener.onProgressChanged(KnobViewPD.this,true, getProgress());
        }
    }
    ...
    private void setAngle(float angle, boolean callback) {
        ...
        if (wheelWidth == 0 || wheelHeight == 0) {
            ...
            if(mProListener != null){
                mProListener.onProgressChanged(KnobViewPD.this,false, getProgress());
            }
        }
    }
        ...
        public boolean onTouch(View v, MotionEvent event) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_DOWN:
                    ...
                    if(mProListener != null){
                        mProListener.onStartTrackingTouch(KnobViewPD.this, getProgress());
                    }
                    if (!touchable) {
                        return false;
                    }
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if(mProListener != null){
                        mProListener.onStopTrackingTouch(KnobViewPD.this, getProgress());
                    }
                    break;
            }
...
//        progress = Math.max(1, progress);
    ...
    public interface OnProgressChangeListener{
        ...
        void onProgressChanged(KnobViewPD view, boolean fromUser, int progress);

        void onStartTrackingTouch(KnobViewPD view, int progress);

        void onStopTrackingTouch(KnobViewPD view, int progress);
    }
}
