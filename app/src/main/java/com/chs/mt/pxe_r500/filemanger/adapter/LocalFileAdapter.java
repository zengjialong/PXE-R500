package com.chs.mt.pxe_r500.filemanger.adapter;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = LayoutInflater.from(context).inflate(
					R.layout.chs_local_file_list_item, null);
		...
		if (files.get(position).isDirectory()) {
			viewHolder.fileIcon.setImageResource(R.drawable.chs_folder);
			viewHolder.fileImage.setImageResource(R.drawable.chs_file_folder);
			viewHolder.textSize.setText("");
		} else {
			viewHolder.fileImage.setImageResource(R.drawable.chs_ic_action_overflow);
