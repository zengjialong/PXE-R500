...
public class KnobView_KTV extends AppCompatImageView {
	...
	public int outside_point_color_press = 0;
	public int outside_point_color_normal = 0;
    ...
    public KnobView_KTV(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        mAttrs = attrs;
        init(context);
    ...
    private void init(Context context) {
        ...
        progressBgPaint.setColor(outside_point_color_normal);
        ...
        progressPaint.setColor(outside_point_color_normal);
        ...
        progressPaint2.setColor(outside_point_color_press);
...
//        progressPaintL.setStrokeCap(Paint.Cap.ROUND);
        ...
        PaintInsidePoint.setStrokeWidth(progressWidth);
        PaintInsidePoint.setStrokeCap(Paint.Cap.ROUND);
    ...
    private void initView(){
        TypedArray localTypedArray = mContext.obtainStyledAttributes(mAttrs, R.styleable.KNOB);
     	...
     	inside_point_radius = localTypedArray.getDimension(R.styleable.KNOB_inside_point_radius, 8);
     	outside_point_radius = localTypedArray.getDimension(R.styleable.KNOB_outside_point_radius, 2);
        
     	inside_point_color = localTypedArray.getColor(R.styleable.KNOB_inside_point_color, getResources().getColor(color.white));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {  
        ...
        min = Math.min(WIND_WIDTH, WIND_HEIGHT);
        //setMeasuredDimension(min, min);  
        centerX = min / 2;
        centerY = min / 2;
        drawableRadius = min / 2;
        progressWidth = 1;//(int)(drawableRadius*PROGRESS_RATIO);
        radius = drawableRadius*0.85f;
        thumbRadius = radius;
        pic_radius=drawableRadius*0.7f;
        
        InsideRadius = (float) (drawableRadius*0.70);
        OutsideRadius= (float) radius;
...
//    	    System.out.println("FUCK scaleHeight:"+scaleHeight);
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (wheelHeight == 0 || wheelWidth == 0) {
            ...
            progressPaint.setStrokeWidth(inside_line_width);
            progressPaint2.setStrokeWidth(inside_line_width);
            progressPaintL.setStrokeWidth(inside_line_width);
            ...
            //float radius2 = radius + shadowWidth;
            rect = new RectF(centerX - InsideRadius, centerY - InsideRadius, centerX + InsideRadius, centerY + InsideRadius);
        }
    }
    @Override
...
//        canvas.drawArc(rect, 0, 360, false, paint);
        super.onDraw(canvas);
        canvas.drawArc(arcRect, START_ANGLE, START_ANGLE+MAX_SWEEP_ANGLE, false, progressBgPaint);
        drawProgress(canvas);
        //drawDrawBg(canvas);
...
//        drawInsideLine(canvas);
    }

    private float sweepAngle = 0;
    private void drawProgress(Canvas canvas){
        canvas.drawArc(rect, START_ANGLE, 360, false, progressPaint);
        canvas.drawArc(rect, START_ANGLE, sweepAngle, false, progressPaint2);
    }
...
//        canvas.restore();
    ...
    private void drawInsidePoint(Canvas canvas) {
    	float[] pointerPosition=calculatePointerPosition(316-sweepAngle, OutsideRadius);
    	canvas.drawCircle(pointerPosition[0], pointerPosition[1], inside_point_radius, PaintInsidePoint);
    }

    private void drawInsideLine(Canvas canvas) {
        float[] start=calculatePointerPosition(316-sweepAngle, InsideRadius);
        float[] end=calculatePointerPosition(316-sweepAngle, OutsideRadius);
        canvas.drawLine(start[0],start[1],end[0],end[1],PaintInsidePoint);
    }
    ...
    private void rotateWheel(float degrees) {
        ...
        if(mProListener != null){
            mProListener.onProgressChanged(KnobView_KTV.this,true, getProgress());
        }
    }
    ...
    private void setAngle(float angle, boolean callback) {
        ...
        if (wheelWidth == 0 || wheelHeight == 0) {
            ...
            if(mProListener != null){
                mProListener.onProgressChanged(KnobView_KTV.this,false, getProgress());
            }
        }
    }
        ...
        public boolean onTouch(View v, MotionEvent event) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_DOWN:
                    ...
                    if(mProListener != null){
                        mProListener.onStartTrackingTouch(KnobView_KTV.this, getProgress());
                    }
                    if (!touchable) {
                        return false;
                    }
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if(mProListener != null){
                        mProListener.onStopTrackingTouch(KnobView_KTV.this, getProgress());
                    }
                    break;
            }
...
//        chs_progress = Math.max(1, chs_progress);
    ...
    private OnKnobView_PicTwo_ProgressChangeListener mProListener;

    public void setProgressChangeListener(OnKnobView_PicTwo_ProgressChangeListener listener) {
        this.mProListener = listener;
    }

    public interface OnKnobView_PicTwo_ProgressChangeListener{

        void onProgressChanged(KnobView_KTV view, boolean fromUser, int progress);

        void onStartTrackingTouch(KnobView_KTV view, int progress);

        void onStopTrackingTouch(KnobView_KTV view, int progress);
    }
}
